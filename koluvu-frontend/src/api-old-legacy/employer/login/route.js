// src.api.employer/login/route.js
...
export async function POST(request) {
  try {
    ...
    const djangoApiUrl = process.env.NEXT_PUBLIC_DJANGO_API_URL || 'http://localhost:8000/api/employer/login/';
    ...
    if (!djangoResponse.ok) {
      ...
      return NextResponse.json({ error: errorData.error || 'Login failed' }, { status: djangoResponse.status });
    }
    ...
    if (supabaseError) {
      ...
      if (supabaseError.code === 'email_not_confirmed') {
        return NextResponse.json({ error: 'Email not verified. Please verify your email before logging in.' }, { status: 400 });
      }
      return NextResponse.json({ error: 'Invalid login credentials.' }, { status: supabaseError.status || 401 });
    }

    if (!supabaseData.user) {
      console.error('No user data returned after sign in');
      return NextResponse.json({ error: 'Failed to login: No user data returned.' }, { status: 500 });
    }
