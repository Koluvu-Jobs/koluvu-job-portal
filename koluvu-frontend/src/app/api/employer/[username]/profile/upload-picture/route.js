...
export async function POST(request, { params }) {
  try {
    ...
    console.log(
      `Form data received for profile picture upload for user: ${username}`
    );
    ...
    const backendUrl = `${
      process.env.NEXT_PUBLIC_BACKEND_URL || "http://127.0.0.1:8000"
    }/api/employer/${username}/profile/upload-picture/`;
    ...
    if (!backendResponse.ok) {
      console.error(
        `Backend profile picture upload failed: ${backendResponse.status}`
      );
