...
export async function POST(req) {
  try {
    ...
    if (resumeFile && resumeFile.size > 0) {
      ...
      resumeFilename = `${uniqueSuffix}-${resumeFile.name.replace(/\s+/g, '_')}`;
    ...
    console.log(`[API Setup] Successfully created session with ID: ${sessionId}`);
    ...
    return NextResponse.json({ error: 'Failed to complete setup.', details: error.message }, { status: 500 });
  }
}
...
export async function GET(request) {
  try {
    ...
    if (setupId) {
      ...
      return NextResponse.json(result.rows[0]);    } else {
