// src.api.employer/register/route.js
...
export async function POST(request) {
  try {
    ...
    if (authError || !authData.user) {
      return NextResponse.json({ error: authError?.message || 'Failed to register user.' }, { status: 500 });
    }
    ...
    const djangoApiUrl = process.env.NEXT_PUBLIC_DJANGO_API_URL || 'http://localhost:8000/api/employer/register/';
    ...
    if (!djangoResponse.ok) {
      return NextResponse.json({ error: djangoResult.error || 'Failed to save employer details in local database.' }, { status: 500 });
    }

    return NextResponse.json({ message: 'Employer registered successfully!', user: authData.user, djangoResult }, { status: 201 });
  } catch (error) {
